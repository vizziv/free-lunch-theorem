...
\begin{centering}
  \begin{longtable}{l p{.65\textwidth} l}
    ...
    $\pfp{v}$ & Periodic firing pattern of $v$. & Section~\ref{motors}
    \vspace{\tablespace}\\
    ...
    $\pfps$ & Set of firing patterns in $\s$. &
    Section~\ref{nonclumpiness} \vspace{\tablespace}\\

    $\cpfps$ & Set of ``clumpy'' firing patterns in $\s$. &
    Section~\ref{nonclumpiness} \vspace{\tablespace}\\
...
We leave out the subscript $G$ or superscript $\s$ where there is no ambiguity.
