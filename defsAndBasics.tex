...
\begin{centering}
  \begin{longtable}{l p{.65\textwidth} l}
    ...
    \S~\ref{pres} \vspace{\tablespace}\\

    $\pfp{v}$ & Periodic firing pattern of $v$. & \S~\ref{nonclumpiness}
    \vspace{\tablespace}\\
    ...
    fires (if $f = 1$) in $\s$. & \S~\ref{motors} \vspace{\tablespace}\\

    $\period$ & Period of $\s$. & \S~\ref{pres} \vspace{\tablespace}\\

    $\mots$ & Set of vertices that are motors in $\s$. & \S~\ref{motors}
    \vspace{\tablespace}\\

    $\pfps$ & Set of periodic firing patterns in $\s$. &
    \S~\ref{nonclumpiness} \vspace{\tablespace}\\
