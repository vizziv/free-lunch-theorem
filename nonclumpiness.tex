...
implies that firing patterns with sector switches are impossible.
...
We consider parallel chip-firing game $\s$ on undirected graph $G$. Recall that
the
firing pattern of a vertex $v \in \V$ is the binary
string
\[
  \firing{v}{t_0}\dots\firing{v}{t_0 + \period - 1},
\]
where $t_0$ is the smallest natural number such that $\pos{t_0}$ is
periodic. We write the firing pattern of $v$ as $\pfp{v}$. For simplicity, we assume here
that $t_0 = 0$ and index firing patterns modulo $\period$.

Let $\pfps$ be the set of all firing patterns occurring in $\s$. Call a firing pattern with both
consecutive 0s and consecutive 1s \emph{clumpy}, and let $\cpfps$ be the set of
all clumpy firing patterns occurring in $\s$. (Recall that the $\period$\xth and 0\xth
entries of a firing pattern are the same, so, for example, 011010 is clumpy.) It is known
that, given any nonclumpy firing pattern, one can construct a parallel chip-firing game on
a simple cycle in which every vertex has that firing pattern shifted by some number of
steps~\cite{cycle}. We prove here that clumpy firing patterns cannot occur in any parallel
chip-firing game, showing that a firing pattern is possible if and only if it is
nonclumpy.
...
\end{align*}
for binary strings $p$ and $q$. We now prove our main result: clumpy firing patterns do
...
with clumpy firing patterns bounds a quantity below, and summing an inequality given by
the Theorem~\ref{morale} over all edges incident with a vertex with a clumpy
firing pattern gives an upper bound on the same quantity. The lower bound is the total
number of vertices with clumpy firing patterns, and the upper bound is 0.
...
firing patterns. Choosing $r_{v,X} = -s_X(\pfp{v})$ and splitting the sum in
\eqref{almost} between neighbors with and without clumpy firing patterns yields
\begin{align*}
